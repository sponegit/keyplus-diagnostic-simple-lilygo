...
void loop()
{
    ...
    // ── 발행 완료 확인 (P3) ─────────────────────────────────────────────────
    // 지금까지 "published" 는 AT 수락까지만 뜻했다 — 실제 전달 결과 URC(+CMQTTPUB)는
    // 파싱조차 안 하고 버렸다. 패치한 래퍼가 그 URC 를 기록하므로, 내보낸 발행의
    // 결과가 제 시간에 안 오면 세션이 죽은 것으로 본다.
    // ⚠️ 반드시 Mqtt::handle() **뒤에** 둔다. 앞에 두면 방금 도착한 결과를 아직 읽지
    //    않은 채 "미도착"으로 오판한다.
    if (Mqtt::pubAckOverdue(modem, now)) {
        noteModemRestarted("발행 결과 URC 미도착");
    }
