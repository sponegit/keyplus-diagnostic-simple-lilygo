...
// 접속 기술 고정 (AT+CNMP). 38=LTE only, 2=자동, 13=GSM only, 51=GSM+LTE.
// GSM 은 TX 가 577us 슬롯에 2A 급으로 몰리는 버스트라, 전원 마진이 얇은 보드에서
// 브라운아웃의 최악 케이스다. LTE 전용으로 고정하면 그 버스트가 아예 사라지고
// 등록 시 GSM 대역 스캔도 건너뛴다. LG U+ 는 2G/3G 를 종료해 잃는 폴백이 없다.
// 0 이면 건드리지 않는다(다른 사업자/로밍 검증 시 되돌리는 스위치).
#define LTE_CNMP_MODE               (38)
// --- 모뎀 무응답 복구 -------------------------------------------------------
