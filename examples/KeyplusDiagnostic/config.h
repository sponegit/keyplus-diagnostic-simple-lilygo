...
// ⚠️ **시험 전용 — 기본값 0 을 파일에서 바꾸지 말 것.**
//    'buf fill <n> [gap_s]' 콘솔로 링에 합성 레코드를 주입한다. 현장 단말에 임의 레코드를
//    밀어넣을 수 있는 명령이 남아 있으면 그 자체가 사고다.
//    용도: 백필 드레인(≈1Hz 연속 발행)을 **차량 없이 벤치에서** 재현하는 것.
//    2026-08-10 조사에서 자발적 절단 3회가 전부 드레인 구간 안에서만 났고 드레인이
//    끝나자 멎었다 — 3일치 버퍼를 실제로 쌓지 않고 그 자극만 만들어 내려는 것이다.
//
//    켜는 법은 **빌드 플래그**다(#ifndef 로 열어 둔 이유):
//      PLATFORMIO_BUILD_FLAGS=-DFEATURE_BUF_TESTFILL=1 pio run -e T-A7670X -t upload
//    파일을 고쳐서 켜면 1 인 채로 커밋·OTA 될 길이 열린다. 그 경로를 아예 막아 둔다.
#ifndef FEATURE_BUF_TESTFILL
#define FEATURE_BUF_TESTFILL 0
#endif
