# Keyplus Diagnostic 펌웨어 0.2.5 — OTA 배포 패키지

| 항목 | 값 |
|---|---|
| 버전 | **0.2.5** (`FW_VERSION`, `config.h`) |
| 보드 | LILYGO T-A7670E (ESP32-WROVER, Flash 4MB) |
| 파일 | `keyplus-diagnostic-0.2.5.bin` |
| 크기 | **385,312 bytes** |
| MD5 | `e8503de00a2b48bee75dd46d996ed4f6` |
| SHA-256 | `a151e00fb4ec6d4c60c858d90a58f15b7fdedb82fc7151f91935f33d4b69ccf3` |
| OTA 슬롯 | 1,966,080 bytes (min_spiffs.csv, **19.5% 사용**) |

단말은 **MD5만** 검증합니다(`Update.setMD5`). SHA-256은 배포 경로 무결성 확인용입니다.

## 0.2.4 대비 변경 (실기 로그에서 확인)

- **`info`/`status` 의 AT 조회가 URC 를 응답으로 오독하던 문제 수정.**
  발행 ACK(`+CMQTTPUB`) 가 스트림에 떠 있을 때 `info` 를 치면 IMEI 자리에
  `+CMQTTPUB: 0,0` 이 찍혔습니다. 진단 도구가 거짓 값을 주던 셈입니다.
  조회 전에 URC 를 먼저 소화시킵니다(`CONSOLE_URC_DRAIN_MS`).
  cmd 수신 URC 는 정상 처리되므로 명령을 잃지 않습니다.
- **재접속 시 cmd 구독이 두 번 나가던 문제 수정.**
  구독 재시도 블록이 접속 상승엣지보다 먼저 실행돼, 이전 세션의 낡은 예약 시각으로
  즉시 한 번 나가고 5초 뒤 또 나갔습니다. 블록을 엣지 처리 뒤로 옮겼습니다.
...
```
keyplus-diagnostic-0.2.5.bin       OTA로 내려보낼 이미지 (app 파티션 전용)
keyplus-diagnostic-0.2.5.bin.md5   MD5 한 줄 (배포 서버 검증용)
...
1. `keyplus-diagnostic-0.2.5.bin`을 HTTP(S)로 접근 가능한 곳에 올린다.
...
0.2.5에서 상한을 크게 올렸습니다. 그래도 무제한은 아닙니다.
...
### 2. `version`은 `0.2.5`과 **정확히** 일치해야 한다
...
0.2.5에서 브라운아웃이 관측된 개체라면 전원 보강 후에 진행하는 게 안전합니다.
...
```
[OTA] 시작 cmd=... ver=0.2.5
  url=https://.../kpd-0.2.5.bin
  md5=e8503de00a2b48bee75dd46d996ed4f6
[OTA] 펌웨어 크기 385312 bytes — 플래시 시작
[OTA] 10% (38531/385312)
...
...
```
============================================================
  Keyplus Diagnostic   fw 0.2.5
  reset      : SW(소프트 리셋/재부팅)
============================================================
[OTA] 새 이미지 확정(mark_valid) ... — 롤백 취소
[OTA] pending ack: cmd=... 기대=0.2.5 실행=0.2.5 → done
```
...
- [ ] `ota_start.json` 직렬화 길이 **< 1,000 bytes** (0.2.5 기준)
- [ ] `version` 필드가 `0.2.5`
- [ ] 대상 단말이 **`cmd` 구독 성공** 상태 — `status server`에서 `[ok]` 확인
      (0.2.5 이전 이미지는 구독 실패 버그가 있어 명령 자체를 못 받습니다)
- [ ] 단말 전원 안정 (브라운아웃 이력 없음 — `info`의 `reset` 확인)
- [ ] 1대 선행 배포 후 확대

> **0.2.1은 사용하지 마세요.** 배포 전 실기 테스트에서 결함이 발견되어 0.2.5로 대체했습니다
> — 모뎀 리셋 후 CMQTT 서비스 플래그가 모뎀 상태와 어긋나 MQTT에 영영 재접속하지 못했습니다.
>
> **0.2.0 → 0.2.5 OTA 주의**: 0.2.0에는 cmd 구독이 간헐적으로 실패하는 버그가 있어
> `ota_start` 명령이 단말에 **도달하지 않을 수 있습니다.** 구독이 안 되는 개체는
> 시리얼로 0.2.5를 먼저 구운 뒤, 이후 버전부터 OTA를 사용하세요.
