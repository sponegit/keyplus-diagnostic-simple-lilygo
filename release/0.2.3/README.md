# Keyplus Diagnostic 펌웨어 0.2.3 — OTA 배포 패키지

| 항목 | 값 |
|---|---|
| 버전 | **0.2.3** (`FW_VERSION`, `config.h`) |
| 보드 | LILYGO T-A7670E (ESP32-WROVER, Flash 4MB) |
| 파일 | `keyplus-diagnostic-0.2.3.bin` |
| 크기 | **384,368 bytes** |
| MD5 | `85fbb8fab288303b8f386ac00a2bbaea` |
| SHA-256 | `50bb0cee023264cf88f9d238c8abf4191d7b6a33863b3c717008072ff6defc68` |
...
- **차키 버튼 기본 누름 유지시간 120ms → 500ms** (`CARKEY_PRESS_MS`).
  `press()`는 이 시간만큼 `delay()`로 블로킹하므로, 그동안 loop가 멈춰
  LED 패턴·콘솔·MQTT URC 펌핑이 정지합니다(1회 500ms).
  콘솔 `lock 300` / `unlock 300` 처럼 1~5000ms 범위에서 일시 변경할 수 있습니다.
...
```
keyplus-diagnostic-0.2.3.bin       OTA로 내려보낼 이미지 (app 파티션 전용)
keyplus-diagnostic-0.2.3.bin.md5   MD5 한 줄 (배포 서버 검증용)
...
1. `keyplus-diagnostic-0.2.3.bin`을 HTTP(S)로 접근 가능한 곳에 올린다.
...
0.2.3에서 상한을 크게 올렸습니다. 그래도 무제한은 아닙니다.
...
### 2. `version`은 `0.2.3`과 **정확히** 일치해야 한다
...
0.2.3에서 브라운아웃이 관측된 개체라면 전원 보강 후에 진행하는 게 안전합니다.
...
```
[OTA] 시작 cmd=... ver=0.2.3
  url=https://.../kpd-0.2.3.bin
  md5=85fbb8fab288303b8f386ac00a2bbaea
[OTA] 펌웨어 크기 384368 bytes — 플래시 시작
[OTA] 10% (38436/384368)
...
...
```
============================================================
  Keyplus Diagnostic   fw 0.2.3
  reset      : SW(소프트 리셋/재부팅)
============================================================
[OTA] 새 이미지 확정(mark_valid) ... — 롤백 취소
[OTA] pending ack: cmd=... 기대=0.2.3 실행=0.2.3 → done
```
...
- [ ] `ota_start.json` 직렬화 길이 **< 1,000 bytes** (0.2.3 기준)
- [ ] `version` 필드가 `0.2.3`
- [ ] 대상 단말이 **`cmd` 구독 성공** 상태 — `status server`에서 `[ok]` 확인
      (0.2.3 이전 이미지는 구독 실패 버그가 있어 명령 자체를 못 받습니다)
- [ ] 단말 전원 안정 (브라운아웃 이력 없음 — `info`의 `reset` 확인)
- [ ] 1대 선행 배포 후 확대

> **0.2.1은 사용하지 마세요.** 배포 전 실기 테스트에서 결함이 발견되어 0.2.3로 대체했습니다
> — 모뎀 리셋 후 CMQTT 서비스 플래그가 모뎀 상태와 어긋나 MQTT에 영영 재접속하지 못했습니다.
>
> **0.2.0 → 0.2.3 OTA 주의**: 0.2.0에는 cmd 구독이 간헐적으로 실패하는 버그가 있어
> `ota_start` 명령이 단말에 **도달하지 않을 수 있습니다.** 구독이 안 되는 개체는
> 시리얼로 0.2.3를 먼저 구운 뒤, 이후 버전부터 OTA를 사용하세요.
